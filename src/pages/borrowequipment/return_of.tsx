...
const ReturnOf = () => {
  ...
  // ดึงข้อมูลจาก API แล้วกรองเฉพาะรายการที่ได้รับการอนุมัติ (borrow_equipment_status === 2)
  const fetchBorrowedItems = async () => {
    try {
      setLoading(true);
      // หาก API รองรับ query parameter status=2 สามารถส่งไปได้เลย
      ...
      if (response.data?.data) {
        const approvedData = response.data.data.flatMap((item: any) =>
          item.borrowequipment_list
            .filter((eq: any) => eq.borrow_equipment_status === 2) // เฉพาะรายการที่อนุมัติแล้ว
            .map((eq: any) => ({
