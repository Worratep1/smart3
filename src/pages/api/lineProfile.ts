				...
				if (events.type === "postback" && events.postback?.data) {
					...
					if (postback.type === 'safezone') {
						...
						if (replyToken) {
							...
							await replyNotification_Temperature({ replyToken, message: 'ส่งคำขอความช่วยเหลือแล้ว' });
					...
					// ---- เพิ่มส่วนนี้เข้าไป ----
						...
						if (replyToken) {
							...
							await replyNotification_Fall({ replyToken, message: 'ส่งคำขอความช่วยเหลือแล้ว' });
						}
					}
						...
						if (replyToken) {
							...
							await replyNotification_HeartRate({ replyToken, message: 'ส่งคำขอความช่วยเหลือแล้ว' });
						}
					}
