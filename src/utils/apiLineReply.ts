...
interface ReplyNotification_Fall {
    replyToken : string;
    message    : string;
    groupLineId   ?: string | null;
}
interface ReplyNotification_HeartRate {
    replyToken : string;
    message    : string;
    groupLineId   ?: string | null;
}
...
interface ReplyNotificationPostbackHeartRate {
    userId              : number;
    takecarepersonId    : number;
    type                : string;
    message             : string;
...
export const replyNotification_Fall = async ({
    replyToken,
    message
}: ReplyNotification_Fall) => {
    try {
        const requestData = {
            to:replyToken,
            messages: [
...
export const replyNotification_HeartRate = async ({
    replyToken,
    message
}: ReplyNotification_HeartRate) => {
    try {
        const requestData = {
            to:replyToken,
            messages: [
                            ...
                            contents: [
                                {
                                    ...
                                    contents: [
                                        {
                                            type      : "span",
                                            text      : "แจ้งเตือนอัตราการเต้นของหัวใจผิดปกติ",
                                            color     : "#FC0303",
                                            size      : "xl",
                                            weight    : "bold",
                                            decoration: "none"
...
export const replyNotificationPostbackHeartRate = async ({
    userId,
    takecarepersonId,
    type,
    message,
...
}: ReplyNotificationPostbackHeartRate ) => {
    try {
        const requestData = {
            to:replyToken,
            messages: [
