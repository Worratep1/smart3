...
export const replyNotificationPostback = async ({
    ...
    try {
        ...
        const messages = [];
        console.log('locationData', locationData);  // log ค่านี้ก่อน push
        // กรณีแจ้งเตือนล้ม และมีพิกัด
       if (type === 'fall' && locationData?.locat_latitude && locationData?.locat_longitude) {
            messages.push({
                type: 'location',
